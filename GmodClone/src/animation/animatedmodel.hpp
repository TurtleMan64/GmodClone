...
public:
    ...
    std::unordered_map<std::string, JointTransform> calculateAnimationPose(Animation* animation, float time);

    // Calculates the poses of each bone by blending together two animations.
    std::unordered_map<std::string, JointTransform> calculateAnimationPose(Animation* animation1, float time1, Animation* animation2, float time2, float blend);

    // Given a pose, applies the joint transforms and stores them in the given joint transforms vector.
    void calculateJointTransformsFromPose(std::vector<Matrix4f>* outJointTransforms, std::unordered_map<std::string, JointTransform>* pose);
...
private:
    ...
    std::unordered_map<std::string, JointTransform> interpolatePoses(Keyframe* previousFrame, Keyframe* nextFrame, float progression);
};
#endif
